...
func runSchema(client *koyfin.Client, session *koyfin.Session, args []string) {
	var assetType, indicatorType string
	fs := newFlagSet("schema")
	fs.StringVar(&assetType, "asset-type", "Equity", "Asset type (currently only Equity is supported)")
	fs.StringVar(&indicatorType, "indicator-type", "", "Indicator type: financials, ratios, forward_estimates, market_data (required for Equity)")
