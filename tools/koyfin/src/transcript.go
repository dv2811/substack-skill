...
func runTranscript(client *koyfin.Client, session *koyfin.Session, args []string) {
	var (
		action, kid         string
		transcriptID, limit int
	)
	fs := newFlagSet("transcript")
	fs.StringVar(&action, "action", "list", "Action: list, get, summary")
	fs.StringVar(&kid, "kid", "", "Koyfin identifier for the stock (required for list)")
	fs.IntVar(&transcriptID, "transcript-id", 0, "Key development identifier (required for get/summary)")
	fs.IntVar(&limit, "limit", 10, "Maximum results for list action (1-64)")
