...
func runSearch(client *koyfin.Client, session *koyfin.Session, args []string) {
	...
	query := fs.String("q", "", "Ticker or ETF name to search for (required)")
	categories := fs.String("categories", "Equity,ETF", "Search categories (comma-separated)")
	primaryOnly := fs.Bool("primary-only", false, "Use primary exchange only")
